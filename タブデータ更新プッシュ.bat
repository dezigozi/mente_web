...
cd /d "%~dp0"
echo =========================================
echo  Mente Jisseki - Tab Data Push
echo =========================================
echo.
git diff --quiet -- public/data/tab_data.csv
if %errorlevel% == 0 (
    echo [NO CHANGES] tab_data.csv has not been modified.
    echo Press any key to exit...
    pause > nul
    exit /b 1
)
echo [1/4] Checking changed files...
git status public/data/tab_data.csv
echo.
...
echo [2/4] Committing...
git add public/data/tab_data.csv
git^ commit -m "data: tab_data.csv updated %TODAY%"
if %errorlevel% neq 0 (
