...
- CSV→Excel（exceljs 動的import、品番ごと色分け・太罫線・Meiryo UI・登録済みはグレー斜体）に変更。build 通過、Excel COMで書式確認済み
- 初版（CSV版）は 75fa981 で push 済み。Excel版は未コミット

## ② 次にやること
- ブラウザで実クリック確認 → commit（`feat: 登録用出力をExcel化`）→ push → deploy
